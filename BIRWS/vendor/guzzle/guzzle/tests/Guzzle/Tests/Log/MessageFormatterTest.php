...
class MessageFormatterTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function setUp() {
		...
		$this->handle = $this->getMockBuilder ( 'Guzzle\Http\Curl\CurlHandle' )->disableOriginalConstructor ()->setMethods ( array (
				'getError',
				'getErrorNo',
				'getStderr',
				'getInfo' 
		...
		$this->handle->expects ( $this->any () )->method ( 'getInfo' )->will ( $this->returnValueMap ( array (
				array (
	...
	public function testUsesResponseWhenNoHandleAndGettingCurlInformation() {
		...
		$response = $this->getMockBuilder ( 'Guzzle\Http\Message\Response' )->disableOriginalConstructor ()->setMethods ( array (
				'getInfo' 
		) )->getMock ();
		$response->expects ( $this->exactly ( 2 ) )->method ( 'getInfo' )->will ( $this->returnValueMap ( array (
				array (
						'connect_time',
						'1' 
				),
