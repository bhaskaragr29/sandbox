...
/*
 * This file is part of the Silex framework. (c) Fabien Potencier <fabien@symfony.com> This source file is subject to the MIT license that is bundled with this source code in the file LICENSE.
 */
...
class ApplicationTest extends \PHPUnit_Framework_TestCase {
	...
	public function testOnCoreController() {
		...
		$app->get ( '/foo/{foo}/{bar}', function (\ArrayObject $foo) {
			...
			return new \ArrayObject ( array (
					'foo' => $foo . $request->attributes->get ( 'bar' ) 
			) );
		} );
	...
	public function testRoutesMiddlewares() {
		...
		$app->get ( '/reached', function () use(&$middlewareTarget) {
			...
			return 'hello';
		} )->before ( $beforeMiddleware1 )->before ( $beforeMiddleware2 )->after ( $afterMiddleware1 )->after ( $afterMiddleware2 );
		
		$app->get ( '/never-reached', function () use(&$middlewareTarget) {
			throw new \Exception ( 'This route shouldn\'t run!' );
		} )->before ( $beforeMiddleware3 )->after ( $afterMiddleware3 );
		
		$result = $app->handle ( Request::create ( '/reached' ) );
		
		$this->assertSame ( array (
				'before_middleware1_triggered',
				'before_middleware2_triggered',
				'route_triggered',
				'after_middleware1_triggered',
				'after_middleware2_triggered' 
		), $middlewareTarget );
	...
	public function testSendFile() {
		...
		try {
			$response = $app->sendFile ( __FILE__, 200, array (
