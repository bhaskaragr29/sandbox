	...
	public function register(Application $app) {
			...
			foreach ( $app ['security.firewalls'] as $name => $firewall ) {
				...
				if ($protected) {
					if (! isset ( $app ['security.context_listener.' . $name] )) {
						...
						$app ['security.context_listener.' . $name] = $app ['security.context_listener._proto'] ( $name, array (
		...
		$app ['security.exception_listener._proto'] = $app->protect ( function ($entryPoint, $name) use($app) {
			return $app->share ( function () use($app, $entryPoint, $name) {
				return new ExceptionListener ( $app ['security'], $app ['security.trust_resolver'], $app ['security.http_utils'], $name, $app [$entryPoint], null, 				// errorPage
		...
		if (isset ( $app ['validator'] )) {
			...
			$app ['validator.validator_service_ids'] = array_merge ( $app ['validator.validator_service_ids'], array (
